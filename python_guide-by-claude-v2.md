...
name, age, city = get_user() #详见 9.list（列表）和 tuple（元组）的区别
```
...
single = (1,)            # 单元素tuple，注意逗号
not_tuple = (1)          # 这只是整数1，不是tuple
result = (1 + 2) * 3     # 括号用于改变运算优先级
x = (5)                  # 括号没有任何作用，x就是5
empty = ()               # 空tuple

# 函数返回"多个值"其实返回的是tuple
def get_coords():
    return 10, 20  # 实际上返回的是 (10, 20)
x, y = get_coords()  # 解包tuple
